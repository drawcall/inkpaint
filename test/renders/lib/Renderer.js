...
class Renderer {
    ...
    constructor(viewWebGL, viewContext2d, parentNode) {
         ...
         * @name stage
         * @type {InkPaint.Container}
         */
        this.stage = new InkPaint.Container();
        ...
        if (this.hasWebGL) {
            /**
             * The WebGL InkPaint renderer.
             * @name webgl
             * @type {InkPaint.WebGLRenderer}
             */
            this.webgl = new InkPaint.WebGLRenderer(
                Renderer.WIDTH,
                Renderer.HEIGHT,
                {
                    view: viewWebGL,
        ...
        /**
         * The Canvas InkPaint renderer.
         * @name webgl
         * @type {InkPaint.WebGLRenderer}
         */
        this.canvas = new InkPaint.CanvasRenderer(Renderer.WIDTH, Renderer.HEIGHT, {
            view: viewContext2d,
            backgroundColor: 0xffffff,
            antialias: false,
            preserveDrawingBuffer: true,
    ...
    run(filePath, callback) {
        ...
        if (typeof code !== "function" && !code.async) {
            callback(
                new Error(
                    "Invalid JS format, make sure file is " +
            ...
            const done = () => {
                if (!this.stage.children.length) {
                    callback(
                        new Error(
                            "Stage has no children, make sure to " +
    ...
    compare(filePath, solutionPath, callback) {
        this.run(filePath, (err, result) => {
            ...
            if (!solution.webgl || !solution.canvas) {
                ...
                this.hasWebGL &&
                !this.compareResult(solution.webgl, result.webgl)
            ) {
