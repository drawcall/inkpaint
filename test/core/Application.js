...
describe('InkPaint.Application', function ()
{
    it('should generate application', function (done)
    {
        expect(InkPaint.Application).to.be.a.function;
        const app = new InkPaint.Application();

        expect(app.stage).to.be.instanceof(InkPaint.Container);
        expect(app.ticker).to.be.instanceof(InkPaint.ticker.Ticker);
    ...
    {
        const app = new InkPaint.Application({ autoStart: false });
    ...
    it('should be able to destroy filter manager gracefully', function ()
    {
        ...
        const sprite = new InkPaint.Sprite(InkPaint.Texture.WHITE);

        app.stage.addChild(sprite);

        const filter = new InkPaint.filters.AlphaFilter();
    ...
    describe('set ticker', function ()
    {
        before(function ()
        {
            this.app = new InkPaint.Application();
        ...
        it('should assign ticker object', function ()
        {
            ...
            expect(ticker.add.args[0][2]).to.be.equal(InkPaint.UPDATE_PRIORITY.LOW);
        });
