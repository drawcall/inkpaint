...
describe("InkPaint.Spritesheet", function() {
    before(function() {
        ...
        this.validate = function(spritesheet, done) {
            spritesheet.parse(function(textures) {
                ...
                const width = Math.floor(
                    spritesheet.data.frames[id].frame.w *
                        spritesheet.baseTexture.sourceScale
                );
                const height = Math.floor(
                    spritesheet.data.frames[id].frame.h *
                        spritesheet.baseTexture.sourceScale
                );

                expect(Object.keys(textures).length).to.equal(5);
                expect(Object.keys(spritesheet.textures).length).to.equal(5);
                expect(textures[id]).to.be.an.instanceof(InkPaint.Texture);
                expect(textures[id].width).to.equal(
                    width / spritesheet.resolution
                );
                expect(textures[id].height).to.equal(
                    height / spritesheet.resolution
                );
                ...
                expect(this.animations)
                    .to.have.property("star")
                    .that.is.an("array");
        ...
        this.parseFrame = function(frameData, callback) {
            ...
            const baseTexture = InkPaint.BaseTexture.fromCanvas(
                document.createElement("canvas")
            );

            baseTexture.imageUrl = "test.png";

            const sheet = new InkPaint.Spritesheet(baseTexture, data);

            sheet.parse(() => {
                const { frame } = sheet.textures;

                expect(frame).to.be.instanceof(InkPaint.Texture);
    ...
    it("should exist on InkPaint", function() {
        expect(InkPaint.Spritesheet).to.be.a.function;
        expect(InkPaint.Spritesheet.BATCH_SIZE).to.be.a.number;
    });
    ...
    it("should create an instance", function() {
        ...
        const spritesheet = new InkPaint.Spritesheet(baseTexture, data);
    ...
    it("should create instance with scale resolution", function(done) {
        ...
        image.onload = () => {
            const baseTexture = new InkPaint.BaseTexture(image, null, 1);
            const spritesheet = new InkPaint.Spritesheet(baseTexture, data);
    ...
    it("should create instance with BaseTexture source scale", function(done) {
        const data = require(path.resolve(this.resources, "building1.json")); // eslint-disable-line global-require
        const baseTexture = new InkPaint.BaseTexture.fromImage(
            data.meta.image,
            undefined,
            undefined,
            1.5
    ...
    it("should create instance with filename resolution", function(done) {
        ...
        image.onload = () => {
            const baseTexture = new InkPaint.BaseTexture(image, null, 1);
            const spritesheet = new InkPaint.Spritesheet(baseTexture, data, uri);
